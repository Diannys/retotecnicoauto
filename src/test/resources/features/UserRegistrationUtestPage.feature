#Autor: DIANNYS GRANADILLO BARRERA
  @stories
  Feature: Technical challenge
    As user, I want to register my account on the uTest portal to
    participate as an independent tester
  @scenario1
  Scenario: Search page to create user in uTest
    Given Than Nany wants to register in uTest
    When she search for the Join Today button on the uTest page and fill in all the requested fields

    Then she manager to register
      |firstName | lastName | email | birthMonth | birthDay | birthYear | cityLocation | postalCode | password | passwordConfirm | mobileOption | mobileModel | systemOperative |
      | Diannys  | Granadillo | ddgrana310@gmail.com | August | 30 | 1994 |   Fonseca, La Guajira | 0000 | Contraseña12345 | Contraseña12345 | Apple | iPhone | iOS 14 |


